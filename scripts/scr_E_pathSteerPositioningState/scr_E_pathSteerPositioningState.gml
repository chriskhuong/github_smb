...
if(!once)
	{
		//picks a position around the target at a firing distance;
		mx = target.x + (dist*cos(random(pi*2)));
		my = target.y + (dist*sin(random(pi*2)));
		once = true;
	}
...
//on any transition, set once back to false;
